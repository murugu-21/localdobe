...
The economics change when editing happens on your own device instead of a server. localdobe's [edit PDF tool](/edit-pdf/) runs entirely in your browser: when you click on existing text and retype it, or add a new text box, the original text is genuinely removed from the page and your replacement is written in its place, using a font chosen to match the width and spacing of the original (Liberation Sans, Serif, or Mono, matched to Arial, Times New Roman, and Courier — the fonts behind most everyday documents), all without a server involved. Rotating and resizing pages works the same way — locally, immediately, with no round trip.
...
It's worth being precise about what this kind of editing actually does, since PDFs don't work like word processor documents. A PDF doesn't store paragraphs you can reflow — it stores individual, precisely positioned runs of glyphs. There's no "text box" to type a new sentence into that will automatically wrap and push surrounding content down. So when you edit a line, the tool removes the original glyphs from the page and writes your new text in their place in a matching font. (In the rare case a page's structure prevents clean removal, it falls back to covering the original with a solid rectangle — and tells you when that happened.)

This is excellent for the edits people actually need most often: fixing a typo, changing a date, updating a name, filling in a blank on a form. It's less suited to rewriting entire paragraphs, since surrounding text won't reflow around a longer replacement. For substantial rewrites, you'd want the original source document and a word processor rather than a PDF patch.
