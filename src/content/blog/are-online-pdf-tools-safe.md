...
The other model — the one localdobe uses — skips the upload step entirely. Operations like merging, compressing, splitting, and watermarking run using JavaScript and WebAssembly directly in your browser tab: your device reads the file, processes it, and writes the result, all without a network request carrying the file's bytes anywhere. You can verify this yourself with your browser's developer tools open to the Network tab — process a file and watch that nothing new goes out over the wire when you click the action button.
