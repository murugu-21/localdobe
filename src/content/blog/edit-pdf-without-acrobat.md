...
So when a PDF editor lets you "edit" existing text, what's actually happening under the hood in localdobe's tool is a remove-and-replace operation: the original text is genuinely deleted from the page's content, and your new text is written in its place in a font chosen to match the original as closely as possible. For localdobe specifically, that's a Liberation font — Sans, Serif, or Mono — chosen because it shares the exact same character widths as Arial, Times New Roman, and Courier respectively. The practical result is that a short edit — swapping "March 3rd" for "March 10th," fixing "recieve" to "receive" — looks seamless, because the new text occupies the same space the old text would have. Because the original text is actually gone (not just hidden under a patch, as many editors do), it can't be recovered by selecting, copy-pasting, or extracting text from the exported file. In the rare case where a page's structure prevents clean removal — some scans and unusually built documents — the tool falls back to covering the original with a solid rectangle instead, and tells you it did so after export.

This approach is well-suited to short, contained edits and less suited to restructuring a paragraph, since the surrounding text won't reflow to accommodate a longer replacement. If you need to substantially rewrite a document's content, the source file (Word, Google Docs, whatever generated the original) plus a proper re-export is the better tool for that job — a PDF editor, Acrobat included, is a patch tool, not a word processor.
