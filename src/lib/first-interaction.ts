// One-shot "the visitor is really here" signal, shared by PostHog's deferred
// SDK load (./analytics.ts) and its session-replay start.
//
...
// user input, which fired this on every page view. Real scrolling always
// arrives through one of the inputs below — wheel or keyboard on desktop,
// touch on phones — so nothing is lost.
...
export function onFirstInteraction(cb: () => void, target: EventTarget = window): () => void {
  ...
  // nothing here calls preventDefault, and browsers already treat window-level
  // wheel and touch listeners as passive.
