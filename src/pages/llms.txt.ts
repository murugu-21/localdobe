...
export async function GET(context: APIContext) {
  ...
  const lines = [
    ...
    '- Positioning: a free alternative to BOTH installing a desktop PDF suite (e.g. Adobe Acrobat) AND uploading files to online converter sites. Nothing to install, nothing to upload.',
    '- Files are never uploaded; processing is in-browser. This is verifiable (tools work offline).',
    '- Signature validation verifies certificate chains locally against Adobe’s publicly published trust list (AATL), bundled with the tool.',
